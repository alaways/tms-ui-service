...
const mode = process.env.MODE || 'business_unit'

const LoginBusiness: React.FC = () => {
  ...
  const { mutate: handleLogIn,isLoading } = useGlobalMutation(url_api.loginBusiness, {
      onSuccess: (res: any) => {
      if (res.statusCode === 200 || res.code === 200) {
        const userData: any = {
          ...
          role: res.data.role,
          id_business_unit: res.data.id_business_unit,
  ...
  const submitForm = useCallback(
    (event: any) => {
      ...
      if (!formData.email || !formData.password) {
        Swal.fire({
          icon: 'info',
          title: t('auth_please_enter_email_password'),
          padding: '10px 20px',
        })
        return
      }
          ...
          <div className={`relative hidden w-full items-center justify-center  lg:inline-flex lg:max-w-[835px] border-[80px] border-solid border-themePrimary`}>
            <div className="">
              <div className="w-48 block lg:w-72 ms-10">
                <img src={`${themeInit.logo.BussinessLogo}`} alt="Logo" className="w-full" />
              </div>
              {/* <p className="text-right text-white text-3xl">Business</p> */}
            ...
            <div className="w-full max-w-[440px] lg:mt-16">
              <div className="mb-10">
                ...
                <p className="text-base font-normal leading-normal text-white-dark">
                  {t('auth_use')} <span className={`text-themePrimary`}>{t('auth_email')}</span>
