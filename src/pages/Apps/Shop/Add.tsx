...
const Add = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle(t('add_shop')))         // 已有 key：添加店铺
  ...
  const SubmittedForm = Yup.object().shape({
    ...
    id_business_unit: Yup.string().required(t('required_field')),  // 已有 key
    ...((isFoundTax === 2) && {
      username: Yup.string().required(t('required_field')),        // 已有 key
      password: Yup.string().required(t('required_field')),        // 已有 key
      password_repeat: Yup.string().oneOf([Yup.ref('password'), null], t('password_mismatch')).required(t('required_field')), // 新 key
    })
  })
                        ...
                        onKeyPress={(e: any) => {
                          if (e.key === 'Enter') {
                            ...
                            if (props.values.tax_id?.length === 13) {
                              ...
                              toast.fire({
                                icon: 'error',
                                title: t('tax_id_required'),             // 新 key：需要输入完整的税号
                                padding: '10px 20px',
                              })
                            }
                            return
