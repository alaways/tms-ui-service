...
const Payment = () => {
  ...
  const { contractID, ins_no, payment_preview } = (location.state || {}) as LocationState
  ...
  const { mutate: fetchData } = useGlobalMutation(url_api.getPayment, {
      onSuccess: (res: any) => {
  ...
  useEffect(() => {
    ...
    const fetchPaymentDetails = async () => {
      ...
      try {
        fetchData({data:{id_installment: ins_no?.id,id_contract: contractID}})
      } catch (error) {
        handleError()
      }
    }
    fetchPaymentDetails()
  }, [dispatch, ins_no, contractID, token, apiUrl])
