...
import ReceiptProductPreviewNew from '../Contract/ReceiveProduct/PreviewNew'
...
const AddEdit = () => {
  ...
  const breadcrumbItems = [
    ...
    { label: pageAction ? 'แก้ไข' : 'เพิ่ม', isCurrent: true },
  ]
  ...
  const { mutate: contractCreate, isLoading: isLoadingCreate } = useGlobalMutation(url_api.contractCreate, {
    onSuccess: (res: any) => {
      if (res.statusCode === 200 || res.code === 200) {
        ...
        navigate('/apps/contract-lease/' + res.data.id + '/' + res.data.uuid)
        location.reload()
      } else {
        toast.fire({
          icon: 'error',
  ...
  useEffect(() => {
    ...
    if (formData.is_completed) {
      dispatch(setSidebarActive(['contract-lease', '/apps/contract-lease/list-complete']))
    }
    else if (statusAction === 5) {
      dispatch(setSidebarActive(['contract-lease', '/apps/contract-lease/list-credit']))
    }
    else if (statusAction > 5) {
      dispatch(setSidebarActive(['contract-lease', '/apps/contract-lease/list-cancel']))
    }
    else {
      dispatch(setSidebarActive(['contract-lease', '/apps/contract-lease/list']))
    }
  }, [statusAction])

  useEffect(() => {
    dispatch(setPageTitle('สัญญา'))
    fetchPayload({})
  ...
  const submitForm = useCallback(
    async (event: any) => {
      Swal.fire({
        title: 'บันทึก',
        text: 'คุณต้องการบันทึกสัญญานี้ใช่หรือไม่?',
        icon: 'warning',
        showCancelButton: true,
  ...
  const submitFormCredit = useCallback(
    async (event: any) => {
      Swal.fire({
        title: 'บันทึก',
        text: 'คุณต้องการบันทึกสถานะสัญญานี้ใช่หรือไม่?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: themeInit.color.themePrimary,
        cancelButtonColor: '#d33',
  ...
  const submitFormLockedDevice = useCallback(
    async (event: any) => {
      Swal.fire({
        title: 'บันทึก',
        text: 'คุณต้องการบันทึกสถานะล็อคนี้ใช่หรือไม่?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: themeInit.color.themePrimary,
        cancelButtonColor: '#d33',
        ...
        html:
          ...
          <input id="password" type="password" type="text" class="swal2-input" style="margin: 0 !important width: 100% font-size: 1rem color: #000000" placeholder="รหัสผ่านผู้ใช้งาน สำหรับยืนยันล็อค/ปลอดล็อคเครื่อง">`,
        preConfirm: () => {
  ...
  const onError = () => {
    toast.fire({
      icon: 'error',
      title: 'รูปภาพประกอบสัญญาเกิน 15 รูป',
      padding: '10px 20px',
    })
  }
                    ...
                    return (
                      <>
                        ...
                        <Form className="space-y-5 dark:text-white custom-select mt-5">
                          <div className={`text-4xl pt-4 font-semibold text-center ${props.values.contract_hire_type_id == 1 ? 'text-themePrimary' : 'text-blue-700'} `}>
                            {props.values.contract_hire_type_id == 1 ? "หนังสือสัญญาเช่าซื้อ" : `หนังสือสัญญาเช่าทรัพย์`  }
                          </div>
