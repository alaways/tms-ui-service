...
const Edit = () => {
  ...
  useEffect(() => {
    dispatch(setPageTitle(t('asset_info')))     // 新 key：资产信息
  ...
  const breadcrumbItems = [
    { to: '/apps/asset/list', label: t('asset') }, // 已有 key：资产
  ...
  const { mutate: assetUpdate, isLoading } = useAssetUpdateMutation({
    onSuccess: async (res: any) => {
      if (res.code === 200 || res.statusCode === 200) {
        ...
        if (uploadPromises.length > 0) {
          ...
          resultsUpload.forEach((item: any) => {
            if (item.code !== 'error') {
              ...
              toast.fire({
                icon: 'error',
                title: t('asset_file_upload_error'),          // 已有 key：文件上传错误
                padding: '10px 20px',
              });
            }
          });
        ...
        toast.fire({
          icon: 'success',
          title: t('edit_success'),                     // 已有 key：编辑成功
          padding: '10px 20px',
        });
      }
    },
  ...
  const onError = () => {
    toast.fire({
      icon: 'error',
      title: t('asset_image_limit_error'),        // 已有 key：图片数量限制错误
      padding: '10px 20px',
    });
  }
  ...
  const SubmittedForm = Yup.object().shape({
    ...
    price: Yup.string().required(t('required_field')).matches(PRICE_REGEX, t('asset_price_invalid')), // 已有 key
  });
        ...
        <Formik initialValues={formData} onSubmit={submitForm} enableReinitialize autoComplete="off" validationSchema={SubmittedForm}>
          {(props) => (
            <Form className="space-y-5 dark:text-white custom-select">
              <div className="text-lg font-semibold ltr:sm:text-left rtl:sm:text-right text-center">{t('asset_info')} </div>
              <div className="input-flex-row">
                <div className="w-full">
                  <InputField label={t('asset_serial_number')}  name="serial_number" type="text" disabled={pageAction} />
                </div>
              </div>
              <div className="input-flex-row">
                <SelectField
              ...
              <div className="input-flex-row">
                <div className="input-container">
                  <label>{t('asset_capacity')} <span className="text-rose-600"> * </span></label>
                  <div className={'relative text-white-dark'}>
                    <CreatableSelect
                      id="capacity"
