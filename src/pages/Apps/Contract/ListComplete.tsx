...
const ListComplete = () => {
    ...
    const { mutate: fetchContractData, isLoading: isLoadingContract } = useGlobalMutation(url_api.contractCompleteFindAll, {
        onSuccess: (res: any) => {
    ...
    const fetchExportCsv = async () => {
        ...
        return fetch(apiUrl + url_api.contractCompleteFindAll, requestOptions)
    ...
    const columns: any = [
        ...
        //     sortable: false,
        //     render: (item: any) =>{
        //         return ((role == 'shop' && item?.is_view == 1))  ? '-' :  <p>{item.is_completed ? t('contract_completed') : t('pending_action')}</p>
        //     }
        // },
        {
    ...
    useEffect(() => {
        dispatch(setPageTitle(t('completed_contract_list')))
    }, [dispatch])
