...
const ColorModelCapacity = () => {
  ...
  const settingType = [
    { id: 1, title: t('asset_name') },
    { id: 2, title: t('asset_color_type') },
    { id: 3, title: t('asset_model_type') },
  ]
  ...
  const { mutate: fetchNameData } = useGlobalMutation(url_api.nameFindAll, {
    onSuccess: (res: any) => {
      dispatch(setPageTitle(`${t('asset_type_settings')} ${type_name}`))
  ...
  const SubmittedForm = Yup.object().shape({
    ...
    is_active: Yup.string().required(t('required_field')),
  })

  const SubmittedFormAssetName = Yup.object().shape({
    name: Yup.string().required(t('required_field')),
    id_asset_name: Yup.string().required(t('required_field')),
    is_active: Yup.string().required(t('required_field')),
  })
  ...
  return (
    <>
      <div className="my-2">
        <Breadcrumbs
          items={[
            { to: '/apps/asset-type/list', label: t('asset_type_list') },
            { label: t('settings'), isCurrent: true },
          ]}
        />
      </div>
      <div className="panel px-0 border-white-light dark:border-[#1b2e4b]">
        <div className="flex items-center justify-between flex-wrap gap-4 px-5 ">
          <p className="text-lg">
            {t('asset_type_settings')}: {type_name}
          </p>
        </div>
      </div>
            ...
            <div className="datatables pagination-padding">
              <DataTable
                ...
                columns={[
                  ...
                  {
                    accessor: 'name',
                    title: currentSettingType.id === 1 ? t('asset_name') : (currentSettingType.id === 2 ? t('asset_color') : (currentSettingType.id === 3 ? t('asset_model') : t('asset_capacity'))),
                    sortable: false,
              ...
              <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95" >
                <Dialog.Panel className="panel border-0 p-0 pb-3 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                  ...
                  <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                    {formDataNames.id || formDataColors.id || formDataModels.id || formDataCapacitys.id ? t('edit') : t('add')}
                  </div>
                  <div className="p-5">
                    <Formik
                      enableReinitialize
                        ...
                        <Form className="space-y-5  mb-7 dark:text-white custom-select">
                          ...
                          <div className="flex justify-end items-center mt-8">
                            ...
                            <button type="submit" className="btn btn-primary ltr:ml-4 rtl:mr-4">
                              {formDataNames.id || formDataColors.id || formDataModels.id || formDataCapacitys.id ? t('save') : t('add')}
                            </button>
                          </div>
                        </Form>
                      )}
