...
async function requestAPI(method: Method, url: string, headers = {}, dataBody: any, responseType: XMLHttpRequestResponseType) {
...
// process.env.BACKEND_URL + url, 原本的
  try {
    console.log(url, 'urlShow')
    const response: any = await fetch('/api/v1' + url, requestOptions).catch((error) => console.error(error))
    if (!response.ok) {
...
export default {
  get(url: string, dataBody = {}, headers = {}, responseType = 'json' as responseType) {
    return requestAPI(METHOD_GET, url, headers, dataBody, responseType)
  },
  post(url: string, dataBody: any, headers = {}, responseType = 'json' as responseType) {
    return requestAPI(METHOD_POST, url, headers, dataBody, responseType)
  },
  patch(url: string, dataBody: any, headers = {}) {
    return requestAPI(METHOD_PATCH, url, headers, dataBody, 'json')
  },
  put(url: string, dataBody: any, headers = {}) {
    return requestAPI(METHOD_PUT, url, headers, dataBody, 'json')
  },
  delete(url: string, dataBody = {}, headers = {}) {
    return requestAPI(METHOD_DELETE, url, headers, dataBody, 'json')
  },
}