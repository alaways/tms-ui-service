...
const CameraSquare: React.FC<CameraOCRProps> = ({ onSubmit,onClose }) => {
  ...
  const ratio = 1 // ขนาดบัตรประชาชนไทย: 1010 x 638
      ...
      if (videoRef.current) {
        ...
        videoRef.current.onloadedmetadata = () => {
          if (videoRef.current) {
            ...
            console.log(cropRef.current.clientWidth - (leftCrop * 2))
            const widthCrop = cropRef.current.clientWidth - (leftCrop * 2)
            const heightCrop = widthCrop
            cropRef.current.style.width = `${widthCrop}px`
            cropRef.current.style.height = `${heightCrop}px`
